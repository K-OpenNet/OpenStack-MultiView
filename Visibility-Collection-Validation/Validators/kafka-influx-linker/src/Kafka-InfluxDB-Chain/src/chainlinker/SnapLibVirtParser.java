...
public class SnapLibVirtParser extends SnapPluginParser {
	public SnapLibVirtParser() {
		...
		// Pattern: /libvirt/(alphanumerical or _ or -)/mem/(mem or swap_in or swap_out or major_fault or minor_fault or free or max or unused or available)
		regexTypeMap.put("^\\/libvirt\\/([0-9]|[a-z]|_|\\-)*\\/mem\\/(mem|swap_in|swap_out|major_fault|minor_fault|free|max|unused|available)$", lClass);
		// Pattern: /libvirt/(alphanumerical or _ or -)/cpu/cputime
		regexTypeMap.put("^\\/libvirt\\/([0-9]|[a-z]|_|\\-)*\\/cpu\\/cputime$", lClass);
		// Pattern: /libvirt/(alphanumerical or _ or -)/cpu/vcpu/(numerical not starting with 0 or 0 itself.)/cputime
		regexTypeMap.put("^\\/libvirt\\/([0-9]|[a-z]|_|\\-)*\\/cpu\\/vcpu\\/(0|[1-9][0-9]*)\\/cputime$", lClass);
