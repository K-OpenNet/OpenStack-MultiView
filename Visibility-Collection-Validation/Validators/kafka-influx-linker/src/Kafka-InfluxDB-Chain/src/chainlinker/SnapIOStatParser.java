...
public class SnapIOStatParser extends SnapPluginParser {
	...
	public SnapIOStatParser() {
		...
		// Pattern: /intel/linux/iostat/device/(alphanumerical(lowercase only))/(rrqm_per_sec or wrqm_per_sec or r_per_sec or w_per_sec or rkB_per_sec or wkB_per_sec)
		regexTypeMap.put("^\\/intel\\/linux\\/iostat\\/device\\/([0-9]|[a-z])*\\/(rrqm_per_sec|wrqm_per_sec|r_per_sec|w_per_sec|rkB_per_sec|wkB_per_sec)$", lfClass);
		// Pattern: /intel/linux/iostat/device/(alphanumerical(lowercase only))/(avgrq-sz or avgqu-sz or await or r_await or w_await or svctm or %util)
		regexTypeMap.put("^\\/intel\\/linux\\/iostat\\/device\\/([0-9]|[a-z])*\\/(avgrq-sz|avgqu-sz|await|r_await|w_await|svctm|%util)$", lfClass);
