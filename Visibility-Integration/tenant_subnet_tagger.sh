...
# Description	: Script for Integating/Mapping neutron network, subnet and tenant for operator+tenant tagging to assist flowcentric visibility
#
...
do
	...
	do
		if [[ $subnetline == *"Network"* ]]; then
			...
			echo "$vlan_id,$project_id,$project_name,$subnet_address" >> tenant.tags
		fi
