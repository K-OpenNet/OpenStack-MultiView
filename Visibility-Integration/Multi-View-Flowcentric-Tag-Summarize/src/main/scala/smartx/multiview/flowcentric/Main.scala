...
object Main {
  ...
  def main(args: Array[String]) {
    Thread.sleep(120000)
    val end_time = LocalDateTime.now().minusMinutes(2)
    val start_time = LocalDateTime.now().minusMinutes(7)
    ...
    val spark = SparkSession.builder()
      .master("local[*]")
      .appName("Flow-centric validation Inspection Summarization")
    ...
    SummarizeLatencyUnderlay(spark)
    SummarizeSystemPhysical(spark)
    SummarizeSystemVirtual(spark)
    DataPlaneInspection(spark)
    ControlPlaneInspection(spark)
    spark.close()

    //Underlay Network Resource-layer Data Summarization
    def SummarizeLatencyUnderlay(spark: SparkSession): Unit = {
      //val formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss")
      //val start_time_a = formatter.format(start_time.minusMinutes(5))
      //Query last 10 minutes data
      val start_time_a = start_time.minusMinutes(5)
      System.out.println(start_time_a)
      ...
      FinalResult.toDF.write.parquet("/home/netcs/Multi-View-Staged/SummarizeUndelayLatency.parquet")
    ...
    def SummarizeSystemPhysical(spark: SparkSession): Unit = {
      ...
      //Save result to parquet file format
      FinalResult.toDF.write.parquet("/home/netcs/Multi-View-Staged/SummarizeSystemPhysical.parquet")
    ...
    def SummarizeSystemVirtual(spark: SparkSession): Unit = {
      ...
      //Save result to parquet file format
      FinalResult.toDF.write.parquet("/home/netcs/Multi-View-Staged/SummarizeSystemVirtual.parquet")
    ...
    def DataPlaneInspection(spark: SparkSession): Unit = {
      ...
      //Save result to parquet file format
      result.toDF.write.parquet("/home/netcs/Multi-View-Staged/SummarizeDataPlane.parquet")
...
*/
    ...
    def ControlPlaneInspection(spark: SparkSession): Unit = {
      ...
      //Save result to parquet file format
      result.toDF.write.parquet("/home/netcs/Multi-View-Staged/SummarizeControlPlane.parquet")

      //result.filter("measurementboxname like 'SmartX-Box-GIST1'").show(1000, false)

      // Stage data to ElasticSearch DataStore
      //EsSparkSQL.saveToEs(result, "cp-staged-flows/summarized")
      //result.saveToEs("cp-staged-flows/summarized")
      result.write.mode("append")
        .format("org.elasticsearch.spark.sql")
        .option("es.mapping.date.rich", "false")
